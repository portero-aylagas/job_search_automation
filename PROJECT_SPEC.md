...
LangGraph is the intended whole-app orchestration architecture. The current
implementation introduces the application-requirements slice first:
...
The `data/runtime/` directory stores runtime application state. The tracked
files under `data/` are templates and bootstrap fixtures. The `outputs/`
...
following or inspecting `apply_url`. It is created from
`application_page_snapshot.json` and stored separately from
...
- page_title
- evidence_matches
- parsed forms and controls
- embedded JSON summaries
- job-preserving signals
- clipped visible text and raw HTML excerpts
- redacted errors

This entity is generated by the read-only `inspect_application_page_agent`
LangGraph node. It preserves page evidence before LLM interpretation and must
redact cookies, auth/session values, CSRF tokens, and similar secrets.
